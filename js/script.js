...
const showProductRange = (event) => {
  printCountriesToList( countries.slice(start, start + qty) );
}

const findMatchingNames = (query) => {
  printCountriesToList( countries.filter( country => country.name.toLowerCase().incudes( query.toLowerCase() ) ) );
}
...
document.getElementById(`population`).pop.addEventListener(`change`, (event) => {

  let val = document.getElementById(`population`).pop.value;

  if (val == 0) {
    printCountriesToList( countries.filter ( country => country.ppl < 30000000 ) )
  } else if (val ==1) {
    printCountriesToList( countries.filter ( country => country.ppl >= 3000000 && country.ppl <= 10000000 ) )
  }
    else if (val ==2) {
